...
const Editadmin = () => {
  const navigate=useNavigate();
    ...
    GetEmployeeRoleById,GetallLocation,GetallDepartmentGroup} = useContext(SignContext);
    const { id } = useParams();
  ...
  return (
    ...
    values.image=image;
                  ...
                  const res=setEditUserRoleValues(id,values.name,values.password,values.email,values.location,values.departmentGroup._id,values.departmentType._id,values.userRole,values.image,values.status)
                    if(res){
                        navigate('/admin-user');
                    }
                    resetForm();
